...
// 데이터베이스 연결 설정
mongoose.connect("mongodb+srv://it-node:g32QmEA7DA7cCTp@ut-node.u13qncj.mongodb.net/?retryWrites=true&", 
);
 ...
 * =====================================================================
 */
router.get("/discussions", discussionsController.index, discussionsController.indexView); // 1. index 라우트 생성 (모든 레코드 보기) = GET /discussions,                index 액션, index 뷰
router.get("/discussions/new", discussionsController.new); // 2. 생성 폼을 보기 위한 요청 처리        = GET /discussions/new,            new 액션
router.post(
  "/discussions/create",
  discussionsController.validate, // strips . from email
  discussionsController.create,
  discussionsController.redirectView); // 3. 생성 데이터의 처리와 결과             = POST /discussions/create,       create 액션, redirectView 뷰
  router.get("/discussions/:id", discussionsController.show, discussionsController.showView); // 4. show를 처리하기 위한 라우트          = GET /discussions/:id,            show 액션, showView 뷰
  router.get("/discussions/:id/edit", discussionsController.edit); // 5. edit를 처리하기 위한 라우트          = GET /discussions/:id/edit,       edit 액션
  router.put(
